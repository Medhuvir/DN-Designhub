...
**New-day addendum — footer stripped down to just a copyright line.** Dan flagged the footer's nav links as wrong and asked for the footer to be nothing but "© DN Creative [year]," bottom-right. All three pages (`index.html`, `resources.html`, `about.html`) had their footer markup reduced from the DN mark + note + link row down to a single `<p class="footer-copy">` — the `.footer-note` and `.footer-links` CSS rules were removed from `style.css` as dead code rather than left unused, and `.footer-inner` simplified from a responsive two-column flex layout to a plain `justify-content: flex-end`. The year isn't hardcoded: `main.js` has a new `initFooterYear()` (called from the same `DOMContentLoaded` handler as `initNav()`) that fills `#footer-year` from `new Date().getFullYear()`, so this never needs a manual edit each January.
